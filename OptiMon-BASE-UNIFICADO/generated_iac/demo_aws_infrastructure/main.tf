...
\n
# EC2 Instance - app-server-1
resource "aws_instance" "app_server_1" {
  ami                    = "ami-0abcdef1234567890"
  instance_type         = "t3.small"
  key_name              = var.key_pair_name
  vpc_security_group_ids = [aws_security_group.app_server_1_sg.id]
  subnet_id             = ""
  ...
  tags = {
    Name = "${var.project_name}-app-server-1-${var.environment}"
    Type = "EC2Instance"
  }
}

# Security Group for app-server-1
resource "aws_security_group" "app_server_1_sg" {
  name_prefix = "${var.project_name}-app-server-1-"
  description = "Security group for app-server-1"
  ...
  tags = {
    Name = "${var.project_name}-app-server-1-sg"
  }
}
\n
